...
module.exports = class BaseExperiment extends Events
  constructor: (options = {}) ->
    super(options)
  ...
  loadVariant: (storageKey) =>
    id = @load()?[storageKey]
    log.debug("BaseExperiment.loadVariant", @id, storageKey, id)
    if id? then @variants[id] else null

  saveVariant: (storageKey, variant) =>
    @loadAndSave (saved) =>
      log.debug("BaseExperiment.saveVariant", @id, storageKey, variant, saved)
      if variant?
        saved[storageKey] = variant.id
      else
        delete saved[storageKey]
      saved

  clearVariant: (storageKey) =>
    @saveVariant(storageKey, null)
